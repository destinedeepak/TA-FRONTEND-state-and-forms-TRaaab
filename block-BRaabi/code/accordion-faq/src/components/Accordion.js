        ...
        <section>
          {data.map((faq, index) => (
            <article className="mb-2">
              ...
              >
                {faq.Q}
               <span className="ml-12">{this.state.activeFAQ === index ? <i class="fas fa-hand-point-down"></i> : <i class="fas fa-hand-point-up"></i>}</span>
              </h2>
              <p className={this.state.activeFAQ === index ? 'bg-secondary border-2 border-black-100 p-4 show' : 'hide'}>
                {faq.A}
              </p>
            </article>
          ))}
